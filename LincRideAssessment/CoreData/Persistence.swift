//
//  Persistence.swift
//  LincRideAssessment
//
//  Created by Adewale Sanusi on 28/02/2025.
//

import CoreData
import MapKit

class PlacesManager: ObservableObject {
    let container: NSPersistentContainer
    @Published var savedPlaces: [FavoritePlace] = []
    
    init() {
        container = NSPersistentContainer(name: "LincRideAssessment")
        container.loadPersistentStores { description, error in
            if let error = error {
                print("ERROR LOADING CORE DATA. \(error.localizedDescription)")
            } else {
                print("Successfully loaded Core Data!")
                self.fetchSavedPlaces()
            }
        }
    }
    
    func fetchSavedPlaces() {
        let request = NSFetchRequest<FavoritePlace>(entityName: "FavoritePlace")
        
        do {
            savedPlaces = try container.viewContext.fetch(request)
        } catch let error {
            print("Error fetching. \(error.localizedDescription)")
        }
    }
    
    func saveFavoritePlace(_ place: MKMapItem) {
        // Check if place already exists
        if !placeExists(place) {
            let newPlace = FavoritePlace(context: container.viewContext)
            newPlace.id = UUID()
            newPlace.name = place.placemark.name
            newPlace.address = place.placemark.title
            newPlace.latitude = place.placemark.coordinate.latitude
            newPlace.longitude = place.placemark.coordinate.longitude
            newPlace.dateAdded = Date()
            newPlace.category = place.pointOfInterestCategory?.rawValue
            
            saveContext()
        }
    }
    
    func placeExists(_ place: MKMapItem) -> Bool {
        savedPlaces.contains(where: {
            $0.name == place.placemark.name &&
            $0.latitude == place.placemark.coordinate.latitude &&
            $0.longitude == place.placemark.coordinate.longitude
        })
    }
    
    func removeFavoritePlace(_ placeID: UUID) {
        if let place = savedPlaces.first(where: { $0.id == placeID }) {
            container.viewContext.delete(place)
            saveContext()
        }
    }
    
    func saveContext() {
        do {
            try container.viewContext.save()
            fetchSavedPlaces()
        } catch let error {
            print("Error saving. \(error.localizedDescription)")
        }
    }
}
/*
struct PersistenceController {
    static let shared = PersistenceController()

    @MainActor
    static let preview: PersistenceController = {
        let result = PersistenceController(inMemory: true)
        let viewContext = result.container.viewContext
        for _ in 0..<10 {
            let newItem = Item(context: viewContext)
            newItem.timestamp = Date()
        }
        do {
            try viewContext.save()
        } catch {
            // Replace this implementation with code to handle the error appropriately.
            // fatalError() causes the application to generate a crash log and terminate. You should not use this function in a shipping application, although it may be useful during development.
            let nsError = error as NSError
            fatalError("Unresolved error \(nsError), \(nsError.userInfo)")
        }
        return result
    }()

    let container: NSPersistentContainer

    init(inMemory: Bool = false) {
        container = NSPersistentContainer(name: "LincRideAssessment")
        if inMemory {
            container.persistentStoreDescriptions.first!.url = URL(fileURLWithPath: "/dev/null")
        }
        container.loadPersistentStores(completionHandler: { (storeDescription, error) in
            if let error = error as NSError? {
                // Replace this implementation with code to handle the error appropriately.
                // fatalError() causes the application to generate a crash log and terminate. You should not use this function in a shipping application, although it may be useful during development.

                /*
                 Typical reasons for an error here include:
                 * The parent directory does not exist, cannot be created, or disallows writing.
                 * The persistent store is not accessible, due to permissions or data protection when the device is locked.
                 * The device is out of space.
                 * The store could not be migrated to the current model version.
                 Check the error message to determine what the actual problem was.
                 */
                fatalError("Unresolved error \(error), \(error.userInfo)")
            }
        })
        container.viewContext.automaticallyMergesChangesFromParent = true
    }
}
*/
